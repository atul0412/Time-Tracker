                            ...
                            <tr key={log._id} className={`hover:bg-gray-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-25'}`}>
                              ...
                              <td className="px-2 sm:px-4 lg:px-6 py-2 sm:py-3 lg:py-4 whitespace-nowrap">
                                <span className={`inline-flex items-center px-1.5 sm:px-2 lg:px-3 py-1 text-xs font-bold rounded-full border-2 ${getStatusColor(log.status)}`}>
                                  ...
                                  {log.status === 'FAILURE' && <AlertCircle className="w-2 h-2 sm:w-3 sm:h-3 mr-1" />}
                                  {log.status}
                                </span>
                              </td>
                            </tr>
            ...
            <div className="p-3 sm:p-6">
              {statsLoading ? (
                <div className="flex justify-center items-center py-12 sm:py-16">
                  <div className="flex flex-col items-center">
                    <div className="animate-spin rounded-full h-8 w-8 sm:h-10 sm:w-10 border-b-2 border-blue-600 mb-4"></div>
                    <span className="text-base sm:text-lg text-gray-600">Loading analytics...</span>
                  </div>
                </div>
              ) : stats ? (
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 sm:gap-8">
                  ...
                  <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg sm:rounded-xl p-4 sm:p-6">
                    ...
                    {stats.summary?.actionStats?.length > 0 ? (
                      <div className="space-y-3 sm:space-y-4">
                        {stats.summary.actionStats.map((stat) => (
                          <div key={stat._id} className="flex items-center justify-between p-3 bg-white rounded-lg">
                            <div className="flex items-center">
                              <div className={`p-1.5 sm:p-2 rounded-lg ${getActionStyle(stat._id)}`}>
                                {getActionIcon(stat._id)}
                              </div>
                              <span className="ml-2 sm:ml-3 text-xs sm:text-sm font-semibold text-gray-900">{stat._id}</span>
                            </div>
                            <div className="flex items-center">
                              <div className="w-16 sm:w-24 lg:w-32 bg-gray-200 rounded-full h-2 mr-2 sm:mr-3">
                                <div 
                                  className="bg-blue-600 h-2 rounded-full transition-all duration-500" 
                                  style={{ width: `${(stat.count / stats.summary.totalLogs) * 100}%` }}
                                ></div>
                              </div>
                              <span className="text-xs sm:text-sm font-bold text-gray-700 w-8 sm:w-12 lg:w-16 text-right">{stat.count.toLocaleString()}</span>
                            </div>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="text-gray-500 text-center py-8 text-sm sm:text-base">No action data available</p>
                    )}
                  </div>

                  {/* UPDATED: Top Users - Now shows userName if available, otherwise falls back to userEmail */}
                  <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg sm:rounded-xl p-4 sm:p-6">
                    <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-4 sm:mb-6 flex items-center">
                    ...
                    {stats.topUsers?.length > 0 ? (
                        ...
                        {stats.topUsers.slice(0, 10).map((user, index) => (
                          <div key={user._id} className="flex items-center justify-between p-3 bg-white rounded-lg">
                            <div className="flex items-center min-w-0 flex-1">
                              <div className="w-6 h-6 sm:w-8 sm:h-8 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0">
                                {index + 1}
                              </div>
                              <div className="ml-2 sm:ml-3 flex flex-col min-w-0">
                                {/* UPDATED: Show userName if available, otherwise show 'Unknown User' */}
                                <span className="text-xs sm:text-sm font-semibold truncate" title={user.userName || 'Unknown User'}>
                                  {user.userName || 'Unknown User'}
                                </span>
                                {/* Show userEmail as secondary info */}
                                <span className="text-xs text-gray-500 truncate" title={user._id}>
                                  {user._id}
                                </span>
                              </div>
                            </div>
                            <div className="text-right flex-shrink-0">
                              <div className="text-xs sm:text-sm font-bold text-gray-900">{user.count} actions</div>
                              {user.lastActivity && (
                                <div className="text-xs text-gray-500">
                                  {formatTimestamp(user.lastActivity).date}
                                </div>
                              )}
                            </div>
                          </div>
                      ...
                      <p className="text-gray-500 text-center py-8 text-sm sm:text-base">No user activity data available</p>
                    )}
                  </div>
                </div>
              ) : (
                <div className="text-center py-12 sm:py-16">
                  <BarChart3 className="w-12 h-12 sm:w-16 sm:h-16 text-gray-300 mx-auto mb-4 sm:mb-6" />
                  <p className="text-lg sm:text-xl text-gray-600">No analytics data available</p>
                </div>
              )}
            </div>
          )}
